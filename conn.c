...
void client_exch_dest(struct ctrl_blk *cb)
{
	...
	client_resp_area_stag[0].size = SERVER_AREA * sizeof(uint64_t);  



	client_req_area_stag[0].buf = (uint64_t) (unsigned long) 
	client_req_area;
	client_req_area_stag[0].rkey = client_req_area_mr->rkey;
	client_req_area_stag[0].size = SERVER_AREA * sizeof(uint64_t);  
	...
	for(i = 0; i < NUM_SERVERS; i++) {
		...
		if(read(sockfd, &server_resp_area_stag[i], S_STG) < 0) {
			fprintf(stderr, "ERROR reading from socket");
		}
		fprintf(stderr, "Client %d <-- Server %d's stag: ", cb->id, i);
		print_stag(server_resp_area_stag[i]);


		if(write(sockfd, &client_req_area_stag[0], S_STG) < 0) {
			fprintf(stderr, "ERROR writing server stag to socket");
		}
		fprintf(stderr, "Server %d --> Client %d stag: ", cb->id, i);
		print_stag(client_req_area_stag[0]);
...
void server_exch_dest(struct ctrl_blk *cb)
{
	...
	for(i = 0; i < NUM_CLIENTS; i++) {
		...
		server_req_area_stag[0].size = SERVER_AREA;  


		server_resp_area_stag[0].buf = (uint64_t) (unsigned long) 
		server_resp_area;
		server_resp_area_stag[0].rkey = server_resp_area_mr->rkey;
		server_resp_area_stag[0].size = SERVER_AREA;  

	
		if(write(newsockfd, &server_req_area_stag[0], S_STG) < 0) {
			fprintf(stderr, "ERROR writing server stag to socket");
		}
		fprintf(stderr, "Server %d --> Client %d req stag: ", cb->id, i);
		print_stag(server_req_area_stag[0]);


		if(write(newsockfd, &server_resp_area_stag[0], S_STG) < 0) {
			fprintf(stderr, "ERROR writing server stag to socket");
		}
		fprintf(stderr, "Server %d --> Client %d resp stag: ", cb->id, i);
		print_stag(server_resp_area_stag[0]);
		...
		if(read(newsockfd, &client_resp_area_stag[i], S_STG) < 0) {
				fprintf(stderr, "ERROR reading from socket");
			}
			fprintf(stderr, "Client %d <-- Server %d's resp stag: ", cb->id, i);
			print_stag(client_resp_area_stag[i]);
		...
		if(connect_ctx(cb, cb->local_qp_attrs[i].psn, 
			cb->remote_qp_attrs[i], cb->qp[i], 1)) {  // Unreliable Connection
